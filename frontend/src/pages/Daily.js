...
function Daily({ userId }) {
  ...
  const fetchTasks = async () => {
    ...
    setTasks(res.data);
    const completed = res.data.filter(t => t.completed === 1).length;
    const streak = res.data.length > 0 ? Math.min(...res.data.map(t => t.streak)) : 0;
    getMotivation(streak, completed, res.data.length);
  };
  ...
  const getMotivation = async (streak, completed, total) => {
    try {
      const res = await axios.post('http://localhost:3000/daily/motivasyon', {
        streak, tamamlanan: completed, toplam: total
      });
      setMotivasyon(res.data.mesaj);
    } catch (err) {
      console.error('Motivation failed.');
    }
  };
    ...
    <div className="main-content">
      ...
      <div className="stats">
        ...
        <div className="stat-card">
          ...
          <div className="stat-value yellow">{bestStreak} days</div>
        </div>
        <div className="stat-card">
      ...
      {motivasyon && (
        <div style={{
          ...
          <div>
            <div style={{ fontSize: '9px', color: '#4a8fe8', letterSpacing: '1px', marginBottom: '3px' }}>DAILY MOTIVATION</div>
            <div>{motivasyon}</div>
          </div>
        </div>
      )}
      ...
      {showForm && (
        <div className="task-form">
          ...
          <div className="form-actions">
            ...
            <button className="add-btn" onClick={handleAdd}>Save</button>
          </div>
        </div>
      )}
        ...
        {filteredTasks.map(task => (
          <div key={task.id} className={`task-item ${task.completed ? 'completed' : ''}`}>
            ...
            <div className="task-info">
              ...
              {task.streak > 0 && (
                <div className="task-meta">🔥 {task.streak} day streak</div>
              )}
            </div>
            {task.tag_name && (
              <span style={{
